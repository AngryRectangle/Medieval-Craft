...
var Theme = {
    ...
    openTheme: function (name, page) {
        page = page || 0;
        alert(name + "_" + page);
        if (openThemeArray.length == 0 || name != openThemeArray[openThemeArray.length - 1].link)
            openThemeArray.push({ link: name, page: page });
        ...
        if (theme) {
            ...
            if (theme.pages.length > page + 1) {
                elements["nextPage"] = { type: "button", x: 850, y: 420, bitmap: "next_page", scale: 3, page: page, link: name, clicker: {
                        onClick: function (container) {
                            ...
                            MC.addTimer(Theme.themeCloser, 20, container);
                        }
                    }
                };
            }
            if (page > 0) {
                elements["prevPage"] = { type: "button", x: 50, y: 420, bitmap: "pre_page", scale: 3, page: page, link: name, clicker: {
                        onClick: function (container) {
                            var link;
                            var page;
            ...
            if (theme.mainPage) {
                elements["homeTheme"] = { type: "button", x: 400, y: 420, bitmap: stringPath.imagePath.homePage, scale: 3, link: theme.mainPage, clicker: {
                        onClick: function (container) {
            ...
            if (openThemeArray.length > 1) {
                elements["lastTheme"] = { type: "button", x: 330, y: 420, bitmap: stringPath.imagePath.lastTheme, scale: 3, clicker: {
                        onClick: function (container) {
            ...
            elements["closeButton"] = { type: "button", x: 550, y: 420, bitmap: stringPath.imagePath.closeBook, scale: 3, clicker: {
                    onClick: function (container) {
            ...
            theme.pages[page].container.openAs(gui);
        }
    },
        ...
        for (var i in string) {
            ...
            elements["Linker" + index] = { type: "button", x: string[i].x, y: string[i].y, scale: 1, page: page, link: linkName, bitmap: BitMap, clicker: { h: 0,
                    onClick: function (container) {
                        ...
                        MC.addTimer(Theme.themeCloser, 10, container);
                    }
                }
            };
...
TextVisual.addText("test2", "Lorem ipsum dolor sit amet, consectetur adip", "STANDART_TEXT");
Callback.addCallback("tick", function () {
...
Medieval.Item = {
    add: function (id, params, describe) {
        ...
        if (describe)
            Item.describeItem(id, describe);
        if (params.translate)
            Translation.addTranslation(params.name, params.translate);
    },
    ...
    addFood: function (id, params, describe) {
        if (!params.values.food)
            params.values.food = 0;
        IDRegistry.genItemID(id);
        Item.createItem(id, params.name, {
            name: params.texture[0],
            meta: params.texture[1] || 0
...
function TileRenderModel(id, data) {
    ...
    this.addBox = function (x, y, z, size, block) {
        this.boxes.push(this.formatBox(x, y, z, (x + size.x), (y + size.y), (z + size.z), block));
    };
    this.createCondition = function (x, y, z, mode) {
        ...
        var condition = {
            ...
            addBox: function (x, y, z, size, block) {
                this.boxes.push(model.formatBox(x, y, z, (x + size.x), (y + size.y), (z + size.z), block));
            },
            tiles: {},
            tileGroups: [],
...
var ae_block = {
    add: function (id, variations, describe, rotate) {
        ...
        if (rotate == false) {
            ...
            Block.createBlockWithRotation(id, variations, Block.createSpecialType(describe));
    }
};
var ae_particle = {
    add: function (x, y, z, id, size, count) {
        ...
        for (var i = 0; i < count; i++)
            Particles.addParticle(x, y, z, id, 0, 0, 0, size);
    }
};
var ae_animation = {
...
renderStick.setPart("body", partObj, {});
...
var grinderConstruction = [{ Block: [
            ...
            { x: 0, y: 1, z: 0, id: [0] }, { x: 0, y: 2, z: 0, id: [0] }
        ], Level: 1 },
    { Block: [
            { x: 0, y: 1, z: -1, id: [85] }, { x: 0, y: 2, z: -1, id: [85] }, { x: 0, y: 3, z: -1, id: [85] }, { x: 0, y: 4, z: -1, id: [85] },
            { x: 0, y: 1, z: 1, id: [85] }, { x: 0, y: 2, z: 1, id: [85] }, { x: 0, y: 3, z: 1, id: [85] }, { x: 0, y: 4, z: 1, id: [85] },
...
Callback.addCallback("ItemUse", function (coords, item, block) {
    if (item.id == ItemID.smallHammer && block.id == BlockID.woodenMill) {
        if (!World.getTileEntity(coords.x, coords.y, coords.z).data.wheelLevel) {
            ...
            if (!wheel.Level)
                Game.message("Мельница построена не правильно");
        }
        else {
...
Callback.addCallback("ItemUse", function (coords, item, block) {
    if (item.id == ItemID.smallHammer && block.id == BlockID.waterWheel) {
        if (World.getTileEntity(coords.x, coords.y, coords.z).data.wheelLevel) {
            ...
            World.getTileEntity(coords.x, coords.y, coords.z).data.biome = World.getBiome(coords.x, coords.z);
            if (wheel.Level)
                Game.message("Колесо построено, следите за ним");
        }
    }
});
...
TileEntity.registerPrototype(BlockID.woodenGate, {
    ...
    tick: function () {
        if (World.getWorldTime() % 40 == 0 && this.data.capacity == 0) {
            ...
            if (verticles) {
                var x = verticles[1].x - verticles[0].x - 1;
                var y = verticles[1].y - verticles[0].y - 1;
                var z = verticles[1].z - verticles[0].z - 1;
...
TextVisual.addText("about_us", "Этот мод создавался ещё с 2014 года, с тех пор вышло несколько версий. Одна из них была на ModPE, другая на Core Engine. Теперь я перешёл на совершенно новый уровень: новве анимации, идеи, механизмы. Почти всё, что есть в этом моде создано одним человеком - SlePE", "STANDART_TEXT");
Theme.newLine(Theme.getThemeByName("about_us"), 30);
TextVisual.addText("about_us", "Группа RG Project:", "STANDART_TEXT");
TextVisual.addText("about_us", "vk.com/medievalcraft_mod", { size: 40, pathTypeface: "res/fonts/goth.ttf", color: android.graphics.Color.BLUE });
Theme.createTheme("barrel", "main_page");
TextVisual.addText("barrel", "В бочках вы сможете хранить жидкость. Бочки бывают многоблочные и одноблочные. Крафт одноблочной бочки, хранящей 16 вёдер ниже.", "STANDART_TEXT");
...
TextVisual.addText("barrel", "Многоблочные бочки нужны для хранения большого количества жидкости. Минимальный размер такой бочки - 4х4х4 блока. По углам стоят блоки железа, стороны сделаны из брёвен, а всё внутреннее пространство заполнено досками. Чтобы бочка заработала, поставьте с одной из сторон вместо бревна краник. Вместимость бочки высчитывается согласно формуле: B*16, где B количество блоков досок внутри бочки. Ниже крафт краника.", "STANDART_TEXT");
CustomElement.addCustom("workbenchRecipe", {
    theme: "barrel",
    input: [
        [{ id: 265 }, oP, { id: 265 }],
...
TextVisual.addText("drill", "А вы хотели сходить в шахту с буром? Я да. Теперь и вы сможете. Для работы просто положите вместе с щаряженной пружиной в сетку крафта. За один сломанный блок потребляется 10Кэ. Когда вам надоест, просто достаньте пружину из бура, положив его в сетку крафта.", "STANDART_TEXT");
var oP = [{ id: 5, data: 0 }, { id: 5, data: 1 }, { id: 5, data: 2 }, { id: 5, data: 3 }, { id: 5, data: 4 }, { id: 5, data: 5 }];
CustomElement.addCustom("workbenchRecipe", {
    theme: "drill",
    input: [
...
TextVisual.addText("energyGenerate", "Есть несколько способов генерации энергии: ветряки и водяные мельницы. Водяные мельницы сожно устанавливать только в реках или окенах (тогда они работают как приливные).", "STANDART_TEXT");
var oP = [{ id: 5, data: 0 }, { id: 5, data: 1 }, { id: 5, data: 2 }, { id: 5, data: 3 }, { id: 5, data: 4 }, { id: 5, data: 5 }];
CustomElement.addCustom("workbenchRecipe", {
...
TextVisual.addText("energyGenerate", "У каждого из генераторов есть несколько уровней, здесь я напишу только об одном из них. Для постройки постройте конструкцию, как на картинках ниже, а за центральным блоком поставьте (на 1 картинке водяное колесо, а на второй мельница). Затем нажмите по главному блоку (тот, что только что поставили) молотком. Если вы всё сделали правильно, вам в чат прийдёт сообщение о правильной постройке. Помните, что чем больше блоков свободно перед мельницей, тем лучше она работает, а под водяным колесом должна быть вода.", "STANDART_TEXT");
...
TextVisual.addText("energyStorage", "Также энергию можно хранить в пружинах. Золотая хранит 4500 Кэ, а железная 3000Кэ. Но при попытке зарядить этой энергией маховик (присев и нажать по нему) вам вернётся только 1/10 часть энергии, хранящаяся в пружине. Именно поэтому пружины лучше использовать как переносной источник энергии или просто аккамулятор для бура.", "STANDART_TEXT");
CustomElement.addCustom("workbenchRecipe", {
    theme: "energyStorage",
    input: [
        [{ id: 265 }, {}, {}],
...
TextVisual.addText("grinder", "Этот многоблочный механизм поможет вам раздробить руду на более мелкие камни, что повысит эффективность добычи в 2 раза. Для начала нужно построить конструкцию, как на фото ниже, для этого вам понадобится дробика, дробильный стол и редуктор.", "STANDART_TEXT");
CustomElement.addCustom("workbenchRecipe", {
    theme: "grinder",
    input: [
        [{}, {}, {}],
...
TextVisual.addText("grinder", "Затем просто нажмите по дробильному столику молотком, подведите с одной из сторон энергию, положите в слот для шестерёнок шестерню и можете начинать дробить! Потребляет 4 Кэ/тик, рецепты ниже.", "STANDART_TEXT");
...
Item.createItem("guideBook", "Guide book", { name: "guide_book", meta: 0 }, { stack: 1 });
...
TextVisual.addText("sawmill", "Может получиться так, что вам лень рубить много деревьев, или их вокруг просто мало, ну или вам просто хочется более эффеутивно расходовать ресурсы. Тут и поможет лесопилка. Она позволяет распилить брёвна не на 4 доски, а на 6! Также в ней можно распилить старые и уже не нужные преметы вроде сундуков и дверей. Чтобы её скрафтить понадобиться пила и другие ресурсы.", "STANDART_TEXT");
CustomElement.addCustom("workbenchRecipe", {
    theme: "sawmill",
    input: [
        [{}, { id: 265 }],
...
TextVisual.addText("sawmill", "Для работы расходуется 1 Кэ/тик. В правые верхний слот нужно положить шестерёнку. При распилке брёвен есть некоторый шанс, чтотвы сможете получить опилки", "STANDART_TEXT");
Theme.createTheme("stoneFurnace", "main_page");
TextVisual.addText("stoneFurnace", "Medieval Craft изменяет печь, делая её более реалистичной. Теперь для того, чтобы что-то расплавить в печи, нужно нагреть её до нужной температуры, для этого придётся сжечь достаточно много топлива. У каждого вида топлива есть минимальная и максимальная температура горения (последнее услвность, которая в послежующих версиях будет исправлена, так как в реальности всё зависит от количество тепла, которое выделяет топливо за единицу времени). Когда в печи нет толпива она остывает. Чтобы потсроить новую печь, сделайте куб из булыжника 3х3х3, а в середине одной из боковых сторон сломайте блок и поставьте печку. Ниже приведены виды топлива, а также печные рецепты. Минимальная температура горения - температура, при которой загорается топливо, максимальная - температура, до которой оно может греть, время горения - врем, за которое сгорает топливо, энергия - количество теплоты, которое выделяет топливо.", "STANDART_TEXT");
...
for (var i in MC.fuelList) {
    ...
    TextVisual.addText("stoneFurnace", Math.max(f.temperature.min, 20) + "°C " + f.temperature.max + "°C " + f.timeBurn / 20 + "c " + f.energy + "Мд", "STANDART_TEXT");
    Theme.newLine(Theme.getThemeByName("stoneFurnace"), 30);
}
...
TextVisual.addText("versionHistory", "К сожалению здесь пусто.", "STANDART_TEXT");
...
Callback.addCallback("ItemUse", function (coords, item, block) {
    if (item.id == ItemID.dw) {
        if (MC.playerGetSneaking(Player.get())) {
            ...
            alert("Current biome is " + World.getBiome(coords.x, coords.z));
        }
    }
});
