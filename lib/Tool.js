...
var Tool = {
	...
	add: function (id, block, properties) {
		...
		if(properties.broke){
			this.itemBrokeParametres.push(id, properties.broke);
		}
		if(properties.damage){
			this.itemBrokeParametres.push(id, properties.damage);
		}
		Item.setMaxDamage (id, properties.durability);
	},
	broke: function (damage) {
		item = Player.getCarriedItem ();
		for(var i in this.itemBrokeParametres){
			if(this.itemBrokeParametres[i].id==item.id){
				if (item.data > Item.getMaxDamage (item.id)) {
			this.itemBrokeParametres[i].defineBroke();
		} else {
			//Player.setCarriedItem (item.id, item.count, item.data, item.enchant);
			this.itemBrokeParametres[i].defineDamage();
		}
			}
		}
		item.data += damage;
